...
public class Base {
	...
	public GiftCertificatesPage giftCertificates;
	...
	public WebDriver enterDetailsIntoRegisterAccountPageFields(WebDriver driver) {
		...
		actions.sendKeys(prop.getProperty("firstName")).pause(Duration.ofSeconds(1)).sendKeys(Keys.TAB)
				...
				.pause(Duration.ofSeconds(3)).sendKeys(Keys.TAB).pause(Duration.ofSeconds(1))
	...
	public WebDriver enterDetailsIntoLoginPageFields(WebDriver driver) {
		...
		prop = CommonUtils.loadProperties();
		Actions actions = new Actions(driver);
		actions.sendKeys(prop.getProperty("sampleEmail")).pause(Duration.ofSeconds(1)).sendKeys(Keys.TAB)
				.pause(Duration.ofSeconds(1)).sendKeys(prop.getProperty("validPassword")).pause(Duration.ofSeconds(1))
				.sendKeys(Keys.TAB).sendKeys(Keys.TAB).pause(Duration.ofSeconds(1)).sendKeys(Keys.ENTER).pause(Duration.ofSeconds(2)).build()
				.perform();
		return driver;
	}
